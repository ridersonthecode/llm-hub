...
# --- Lessons Learned -------------------------------------------------------
# Dünne, fest auf RagConfig.lessons_learned_collection gescopte Wrapper um
# rag.add_text/rag.search (statt die generischen rag_*-Tools oben mit
# Collection-Name von Hand zu benutzen) - gedacht für KI-Agenten mit
# Tool-Zugriff (z.B. Qwen in VS Code Copilot Chat, Agent-Modus, siehe
# .vscode/mcp.json), die während der Arbeit an diesem Projekt selbst gelöste
# Fehler/Korrekturen ablegen bzw. vor einem bekannt wirkenden Fehler zuerst
# nachschauen sollen. Siehe auch RagConfig.lessons_learned_auto_inject in
# config.py: dieselbe Collection wird zusätzlich bei jeder Chat-Anfrage
# automatisch durchsucht (Sicherheitsnetz, falls ein Agent search_lessons
# nicht selbst aufruft).

@mcp.tool()
async def remember_lesson(
    mistake: str, correction: str, context: Optional[str] = None,
    tags: Optional[list[str]] = None,
) -> dict:
    """Speichert eine gelöste Lektion (Fehler + Korrektur) projektweit in der
    Lessons-Learned-Wissensdatenbank, damit sie beim nächsten ähnlichen
    Problem sofort wiedergefunden wird (auch von anderen Sitzungen/Agenten).

    Aufrufen, sobald ein Fehler (z.B. ein fehlgeschlagener Terminal-/
    PowerShell-Befehl, ein Build-Fehler o.ä.) erfolgreich behoben wurde, oder
    wenn der Nutzer eine vorherige Annahme/Antwort explizit korrigiert hat.
    NICHT bei jedem Fehler aufrufen - nur wenn die Lehre verallgemeinerbar
    ist (nicht nur für diesen einen Moment relevant).

    - mistake: was schiefgelaufen ist (Fehlermeldung/Symptom, kurz)
    - correction: die tatsächliche Lösung/Korrektur
    - context: optional, zusätzlicher Kontext (z.B. betroffene Datei, Befehl,
      Umgebung)
    - tags: optionale Schlagwörter zum späteren Filtern (z.B. ["powershell", "gpu"])
    """
    cfg = get_config()
    text = f"Fehler: {mistake}\nKorrektur: {correction}"
    if context:
        text += f"\nKontext: {context}"
    return await rag.add_text(
        cfg.rag.lessons_learned_collection, text, source="lesson",
        metadata={"type": "lesson", "tags": tags or []},
    )


@mcp.tool()
async def search_lessons(query: str, top_k: int = 5) -> dict:
    """Durchsucht die Lessons-Learned-Wissensdatenbank nach ähnlichen früheren
    Fehlern/Korrekturen (semantische Suche). Vor dem Angehen eines bekannt
    wirkenden Fehlers aufrufen, um zu prüfen, ob es dafür bereits eine
    dokumentierte Lösung gibt."""
    cfg = get_config()
    col = cfg.rag.lessons_learned_collection
    return {"collection": col, "results": await rag.search(col, query, top_k)}
